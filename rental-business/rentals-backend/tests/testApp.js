...
import { Product, DeliveryOption, CartItem, Order, QuoteRequest, sequelize, seedDatabase } from './setup.js';
...
export function buildApp() {
  ...
  quotesRouter.post('/', async (req, res) => {
    ...
    if (quote.fulfillmentType === 'delivery' && !quote.deliveryAddress?.trim()) {
      return res.status(400).json({
        success: false,
    ...
    try {
      await QuoteRequest.create({
        ...
        customerEventType: customer.eventType?.trim() || null,
        customerGuestCount: customer.guestCount?.toString().trim() || null,
        customerNotes: customer.notes?.trim() || null,
        rentalStart: quote.rentalDates?.start || null,
        rentalEnd: quote.rentalDates?.end || null,
        fulfillmentType: quote.fulfillmentType || 'pickup',
        deliveryAddress: quote.deliveryAddress?.trim() || null,
        items: quote.items,
      });
